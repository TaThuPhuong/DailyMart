...
interface ProductDao {
    ...
    @Query(
        "SELECT p.id,p.name,p.barcode,p.category_id,p.img_product,p.supplier_id,p.unit, pp.sell_price,pp.import_price,pp.effective_date\n" +
                "FROM products p \n" +
                "INNER JOIN (\n" +
                ...
                "ON latest_price.product_id = pp.product_id AND latest_price.max_time = pp.effective_date group by  p.id"
    )
